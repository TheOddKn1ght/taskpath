...
function entities(text) {
  return text.replace(/&(#x[0-9a-f]+|#\d+|amp|lt|gt|quot|apos|nbsp);/gi, (all, name) => {
    if (name[0] !== '#') return ({ amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", nbsp: '\u00a0' })[name.toLowerCase()] || all;
...
export function parseMarkdown(markdown) {
  ...
  const tasks = []; let section = 'later', ignoredBlocks = 0;
  function addTask(node) {
    const parts = node.tokens.filter(t => t.type !== 'checkbox');
    const first = parts.shift();
    const task = { title: first ? plain(first).trim() : '', status: node.checked ? 'done' : section };
    const notes = [], nested = [];
    for (const part of parts) {
      ...
      for (const item of part.items) {
        ...
        } else task[key] = key === 'category' ? match[2].trim().toLowerCase() : match[2].trim();
      }
    }
  ...
  function visit(node) {
    if (node.type === 'heading') section = ({ later: 'later', 'this week': 'week', today: 'today', done: 'done' })[plain(node).trim().toLowerCase()] || 'later';
