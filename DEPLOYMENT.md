...
Continue only if the image build succeeds. The named volume survives image replacement, including accounts, task ciphertext and push signing keys. The React frontend update keeps the existing SQLite schema and encrypted browser queues unchanged. No account invitation or password change is needed for this update. Docker performs the frozen dependency install and client minification; a container restart alone will not pick up new source files or `.env` values.
