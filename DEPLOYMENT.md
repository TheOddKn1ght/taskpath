...
Continue only if the image build succeeds. The named volume survives image replacement, including accounts, task ciphertext and push signing keys. Existing multi-user databases receive the additional push tables on startup. No account invitation or password change is needed for this update. Docker performs the frozen dependency install and client minification; a container restart alone will not pick up new source files or `.env` values.

6. Repeat the HTTPS checks in step 8. Existing Nginx `/api/sync` and `/api/events` configuration is sufficient for this update.
7. On **every device**, open Taskpath online so it can download the new PWA shell, close all Taskpath tabs and installed-app windows, then reopen and unlock. Keep browser storage intact. The worker waits for old windows to close before activating; refreshing one tab may not be enough.
