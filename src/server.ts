import { clientAsset } from './client-assets.ts';
import { Realtime, type RealtimeData } from './realtime.ts';
import { resolve } from 'node:path';
import { SOURCE_DIRECTORY, sourceRelease, releaseForDirectory } from './client-release.ts';
import { AuthManager } from './auth.ts';
import { InputError, Store, object } from './store.ts';
import { PushService } from './push.ts';
import { FileRepository } from './db/files.ts';
import { decodeFile, fileEnvelope, fileQuota, FILE_MAX, FILE_HEADER_MAX } from '../public/file-format.ts';
import { validId } from '../public/crypto.ts';
...
// WebSocket upgrade: Deno's upgradeWebSocket() returns { socket, response };
// the handler returns the response while Realtime owns the socket.
export type Upgrade = (request: Request, data: RealtimeData) => Response | undefined;

export function createHandler(store: Store, auth = new AuthManager(store.db), publicOrigin?: string, realtime?: Realtime, assetDirectory = resolve(here, getEnv('NODE_ENV') === 'production' ? '../dist/public' : '../public'), push = new PushService(store), quota = fileQuota(getEnv('TASKPATH_FILE_QUOTA_MB'))) {
  const fixedRelease = resolve(assetDirectory) === SOURCE_DIRECTORY ? null : releaseForDirectory(assetDirectory);
  const files = new FileRepository(store.db, quota);
  ...
  return async (request: Request, upgrade?: Upgrade): Promise<Response | undefined> => {
    try {
      ...
      const body = async () => {
        ...
        try { return object(JSON.parse(new TextDecoder().decode(concat(chunks)))); } catch (error) { if (error instanceof InputError) throw error; throw new InputError('Invalid JSON.'); }
      };
      ...
      if (path === '/api/files' || path.startsWith('/api/files/')) {
        ...
        if (fileUpload) {
          ...
          let decoded;
          try { decoded = decodeFile(concat(chunks) as Uint8Array<ArrayBuffer>, userId, vaultId); } catch { throw new InputError('Invalid encrypted file.'); }
      ...
      if (path === '/api/events' && request.method === 'GET') {
        ...
        if (!realtime || !upgrade || request.headers.get('upgrade')?.toLowerCase() !== 'websocket') return json({ error: 'WebSocket upgrade required.' }, 426);
        // Deno consumes the request on upgradeWebSocket, so snapshot the
        // session cookie now; revalidation still reads live session state.
        const cookieHeader = request.headers.get('cookie') ?? '';
        const sessionRequest = () => new Request(url, { headers: { cookie: cookieHeader } });
        const upgraded = upgrade(request, { userId, authorized: () => auth.identity(sessionRequest()) === userId });
...
if (import.meta.main) {
  const quota = fileQuota(getEnv('TASKPATH_FILE_QUOTA_MB'));
  if (getEnv('NODE_ENV') === 'production') {
    releaseForDirectory(resolve(here, '../dist/public'));
  }
  if (getEnv('TASKPATH_PASSWORD') || getEnv('TASKPATH_PASSWORD_HASH') || getEnv('TASKPATH_USERNAME')) throw new Error('Remove legacy authentication environment variables. This version uses browser-based encrypted setup and a fresh database.');
  const store = new Store(getEnv('DATABASE_PATH') || './data/taskpath-accounts.sqlite', undefined, getEnv('TASKPATH_TIMEZONE'));
  const auth = new AuthManager(store.db, Number(getEnv('TASKPATH_SESSION_DAYS') || 30));
  const realtime = new Realtime();
  const push = new PushService(store, getEnv('TASKPATH_PUSH_SUBJECT') || (getEnv('TASKPATH_ORIGIN')?.startsWith('https:') ? getEnv('TASKPATH_ORIGIN') : undefined));
  const handler = createHandler(store, auth, getEnv('TASKPATH_ORIGIN'), realtime, undefined, push, quota);
  const upgrade: Upgrade = (request, data) => {
    const { socket, response } = Deno.upgradeWebSocket(request);
    realtime.attach(socket, data);
    return response;
  };
  const hostname = getEnv('HOST') || '127.0.0.1', port = Number(getEnv('PORT') || 3000);
  const server = Deno.serve({ hostname, port, handler: (req) => handler(req, upgrade) as Promise<Response> });
  push.start();
  const origin = getEnv('TASKPATH_ORIGIN') || `http://${hostname === '0.0.0.0' ? 'localhost' : hostname}:${port}`;
  console.log(`Taskpath is ready at ${new URL('/', origin)}`);
  console.log('Create an invitation with: deno task admin invite');
  const shutdown = async () => { realtime.close(); await push.stop(); await server.shutdown(); store.close(); Deno.exit(0); };
  try {
