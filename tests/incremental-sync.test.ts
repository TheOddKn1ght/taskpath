...
import {fixture,testUserId,testVault,login,origin} from './auth-helpers.ts';
import {acceptEncrypted} from '../public/offline.ts';
import type {EncryptedRecord,Envelope} from '../public/types.d.ts';

const zeros = (n: number): string => btoa(String.fromCharCode(...new Uint8Array(n)));
...
const envelope = (i:number, extra:Partial<Envelope> = {}):Envelope => ({
  ...
  editedAt:'2026-01-01T00:00:00.000Z',nonce:zeros(12),
  ciphertext:zeros(32),...extra,
});
...
test('pages respect ciphertext byte bounds and invalid envelopes do not advance local cursors',async()=>{
  const {store}=await fixture();
  try {
    store.sync(testUserId,input(Array.from({length:30},(_,i)=>envelope(i,{ciphertext:zeros(65536)}))));
    const page=store.syncBoard(testUserId);
    expect(page.rows.length).toBeLessThan(50);expect(page.hasMore).toBe(true);
    expect(new TextEncoder().encode(JSON.stringify(page.rows)).byteLength).toBeLessThan(1024*1024);
