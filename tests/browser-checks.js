import { network, localState, activate, lock, clearMemory, isUnlocked, readBoard, offlineRequest, sync, restoreRemembered } from '/assets/e2ee-v3/offline.js';
import { openDatabase, commit, rememberedKey } from '/assets/e2ee-v3/persistence.js';
...
try {
  ...
  const { config } = await network('/api/auth/config'), unlocked = await unlockVault(password, config);
  await network('/api/auth/login', 'POST', { credential: unlocked.credential, revision: config.revision });
