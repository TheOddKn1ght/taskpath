...
test('real sockets notify both devices after commits; reads/retries do not loop; logout and expiry close sockets', async () => {
  ...
  try {
    ...
    const { task } = await api('/api/tasks', 'POST', { title: 'From laptop' });
    ...
    const change = { changeId: 'phone-edit', editedAt, task: { ...task, title: 'From phone', updatedAt: editedAt } };
    ...
    expect((await api('/api/sync')).tasks[0].title).toBe('From phone');
    await api('/api/sync', 'POST', payload, cookieB);
